...
- **第一批 ✅（2026-07-07）**：
  ...
  3. **Esc 统一退出工具**——裁剪/放置蒙版/WB 吸管/范围取样/图章/画笔/
     框选一键全退（画布与取样点保留可续）。
- 待做候选（按价值排序，未开工）：蒙版 hover 即预览覆盖（现为选中显示）；
  滑杆 Shift 微调/滚轮步进；直方图上高光/阴影三角形指示灯（与 J 联动）；
  工具激活时光标形状全覆盖；masks-cleared develop 参考缓存（叠加刷新
  从 2×develop 降到 1×）；批量导出进度条（现为 toast 汇总）。
- 快捷键现状：Ctrl+Z/Y/O/E/S、←/→ 走图、B 对比、O 叠加、J 削波、Esc 退出、
  空格/中键平移、滚轮缩放、双击 Fit↔1:1。
