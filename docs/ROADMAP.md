...
- 更早已上线：反推配方（`fit.rs` + CLI `match`）、gpt-image-2 弹性高分辨率
  ...
  拖拽/持久化/折叠分组/双击归零）。
- **下一步按 §与 Photoshop 的核心差距 的顺序：F 导出管线 → E 高分预览 →
  C 镜头校正 → D 色管 → G 版本（A② AI 分割可穿插）。**
- 待用户真机验收：曲线拖拽/吸管/图章/拉直/范围蒙版手感；「以此母版继续
  修图」链路（修补→动滑杆→再修补→导出）；持久化"正常关闭→重启恢复"；
  范围蒙版 XMP 在真 Lightroom 打开的效果；高分辨率生成与风格提示词的
  真实 API 行为（付费调用，有 400 回退兜底）。
...
E（高分预览）→ C（镜头校正）→ D（色管）→ G（版本）；A②（AI 分割）可穿插。
