...
> 开工。更新于 2026-07-07（**v0.5.2 已发布**，全部已推送 origin == 本地；
> 进入真机反馈驱动阶段——用户试用 → 报障/提需 → 修复/打磨 → 发布）。

## 当前状态（已完成，勿重做）

- **母版续接双重烹饪修复（2026-07-07，用户真机报障"反推成这样"+霓虹橙蓝
  截图，提交 `2fc9092`，未发布）**：流程 生成出片→反推→「以此母版继续修图」
  把"中性底→生成图观感"的拟合配方（饱和 +60 等）**再叠回生成图本身**——
  `Msg::Retouched` 无条件记母版 + `continue_from_master` 无条件保配方，
  其合法性前提"母版=中性显影+局部修补像素"对 reimagine（整幅风格化重绘，
  观感已烘焙进像素）不成立。真对消融渲染逐位复现截图（配方叠生成图=霓虹；
  配方用于 ARW 中性底=与 XMP 一致的正常渲染，置信度 0.46 诚实平庸）。修复：
  `master_restyled` 标记（复用 generated.is_some() 整幅重绘判据）；重绘母版
  续接时配方重置为中性 + 一次性 `open_note` 状态栏解释（否则读作"编辑丢了"）；
  修补母版保留语义不变；按钮悬浮提示按母版种类切换。gui.rs +52/−18。
...
- 待用户真机验收（v0.3.0 起累计）：曲线拖拽/吸管/图章/拉直/范围蒙版
  ...
  **2fc9092 新增待验（未发布）**：生成出片→反推→「以此母版继续修图」全链
  ——滑杆应重置为中性、状态栏出现"观感已烘焙在像素里"提示、预览=生成图
  本身不再霓虹；fill/heal/clone 母版续接仍保留配方（语义不变）。
